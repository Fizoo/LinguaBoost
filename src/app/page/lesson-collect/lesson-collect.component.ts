...
@Component({
  selector: 'app-lesson-collect',
  templateUrl: './lesson-collect.component.html',
  styleUrls: ['./lesson-collect.component.scss'],
  changeDetection:ChangeDetectionStrategy.OnPush
})
export class LessonCollectComponent implements OnInit,OnDestroy{
  ...
  collect(word: string) {
    ...
    this.sentenceArray=this.sentenceArray.filter(el=>!el.includes(word))
  }

  getTempSentence(value: string){
    console.log(value)
   let x=value.split(' ')
    console.log(this.sentenceArray)
   // console.log(x)
    this.sentenceArray=x.sort(()=>Math.round( Math.random()*100)-50)
